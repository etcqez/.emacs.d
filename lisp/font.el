(defun my-apply-fonts ()
  ...
  (let ((en-font "JetBrains Mono")
        (cn-font "HarmonyOS Sans SC"))
    
    (when (display-graphic-p)
      ;; 1. 设置英文
      (set-face-attribute 'default nil :family en-font :height 110 :weight 'normal)
      
      ;; 2. 设置中文
      (dolist (charset '(han kana bopomofo cjk-misc))
        (set-fontset-font t charset 
                          (font-spec :family cn-font :weight 'medium)))
      
      ;; 3. 缩放比例
      (setq face-font-rescale-alist `((,cn-font . 1.0)))
      
      (message "字体已自动应用: %s + %s" en-font cn-font))))

;; 关键：无论已经开了窗口，还是以后要开窗口，都确保运行
(if (daemonp)
